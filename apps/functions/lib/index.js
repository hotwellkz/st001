...
async function audit(actorUid, action, detail) {
...
export const adminSetKillSwitch = onCall(async (request) => {
    ...
    await db
        ...
        .doc(INSTANCE_ID)
        .set({ emergencyHalt: halt, updatedAt: FieldValue.serverTimestamp(), instanceId: INSTANCE_ID }, { merge: true });
...
export const adminSetTradingMode = onCall(async (request) => {
    ...
    if (mode === "live") {
        ...
        if (!ack) {
            throw new HttpsError("failed-precondition", "Live requires ackLive:true and LIVE_TRADING_ENABLED on engine");
        }
    }
